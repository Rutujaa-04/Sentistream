...
@pytest.mark.asyncio
async def test_worker_drift_alerting_and_slack():
    import asyncio
    worker = ConsumerWorker("test_worker")
    worker.redis_client = AsyncMock()
    worker.price_feed = AsyncMock()
    worker.strategy = MagicMock()
    worker.strategy.should_trade.return_value = None
    
    # Mock model
    worker.model = MagicMock()
    worker.model.infer.return_value = {"label": "positive", "confidence": 0.9, "latency_ms": 10.0, "tokenization_latency_ms": 1.0}
    
    # Create a real DriftDetector but mock its update to return a drift_signal
    worker.get_drift_detector = MagicMock()
    mock_detector = MagicMock()
    drift_signal = MagicMock()
    drift_signal.z_score = 2.5
    drift_signal.window_mean = 0.1
    drift_signal.window_std = 0.3
    drift_signal.direction = "bullish_spike"
    drift_signal.triggered_threshold = 2.0
    mock_detector.update.return_value = drift_signal
    worker.get_drift_detector.return_value = mock_detector
    ...
    worker.send_slack_alert = AsyncMock()
    
    # Process message
    fields = {
        "id": "mock-uuid-2",
        "ticker": "AAPL",
        "headline_text": "AAPL beats earnings estimations by a lot",
        "source": "mock_source",
        "ingested_at": str(time.time())
    }
    
    await worker.process_message("message-id-2", fields)
    
    # Assertions
    # 1. Database insert_drift_alert should have been called
    worker.db.insert_drift_alert.assert_called_once()
    # 2. send_slack_alert should have been triggered
    await asyncio.sleep(0.05)
    worker.send_slack_alert.assert_called_once_with(
        ticker="AAPL",
